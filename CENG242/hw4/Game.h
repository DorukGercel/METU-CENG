...
public:
  ...
  bool captureChest() const;
  void sortVector();                                         //sorts the player vector according to ID

  std::vector<Player*> attackableEnemies(Player* p);        //returns vector of enemies in the attackable range
  Player* attackEnemy(std::vector<Player*> v);              //returns the pointer to the enemy to attack, it can return NULL

  std::vector<Player*> heallableAllies(Player* p);          //returns vector of allies in healable range

  std::vector<Coordinate> goToChestCoords(Player* p);       //returns Coords that get you closer to chest, last element is player's Coord
  Coordinate goChestCoord(std::vector<Coordinate> v);       //returns Coord to move for chest, if can't move returns curent Cord

  Player* getCloseEnemy(Player* p);                         //choose the closest enemy
  std::vector<Coordinate> goToEnemyCoords(Player* p, Player* enemy);    //choose the closer cords to enemy, last element is current coord
  Coordinate goEnemyCoord(std::vector<Coordinate> v, Player* enemy);    //choose the cord to move, if fail returns the current cord

  Player* getCloseAlly(Player* p);                          //choose the closest ally
  std::vector<Coordinate> goToAllyCoords(Player* p, Player* ally);       //choose the coords that are closer to ally, last element is current cord
  Coordinate goAllyCoord(std::vector<Coordinate> v, Player* ally);       //choose the cord to move, if fail returns curent cord

  Goal goalMaker(Player* p, Goal goal, int no_Priority);
