...
public:
    ...
    Car* getHeadCar() const;
    Car* getLastCar() const;

    void setHeadCar(const Car* car);

    void deleteLaps(Car* cars);
    void cleanLapper(Laptime* laps);

    void bubbleSortCars();

    std::string fastestCarName() const;
    int digit_maker() const;

    Race();
    void pointArranger();

    void bubbleSortPoints();

    void setAvgLaptime(const Laptime laptime);

    void setNumberOfCars(int no);
