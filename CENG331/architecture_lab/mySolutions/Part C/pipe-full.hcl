...
####################################################################
# Description:(Same description in Part B)
####################################################################
# iaddq V, rB
#
#Fetch:
...
#	PC <- valP
####################################################################
# In fetch stage we first take instruction byte (1 byte), register byte(1 byte),
#value bytes (8 bytes), from our instruction memory. As our instruction is 10 bytes
#we increment our PC with 10. This is a valid instruction therefore we add this 
#instruction to list of inst_valid, and as we use both rsgister and constant in
#this instruction we add it to the list of need_regids and need_valC.
# In decode stage we will need to take the value of register rB. Therefore we need
#to set the value of srcB to rB if icode is IIADDQ so we added to the list and also
#we are gonna store the evaluated value to rB therefore, dstE is rB when icode is
#IIADDQ.
# In execute stage we are going to sum a constant with a register data, therefore
#the value we choose for aluA is valC and for aluB it is valB. Also we set condition
#codes if necessary for this operation.
# We don't do anything in memory stage.
# We write back to dst srcB in Write Back stage.
# By default we choose our PC as already incremented valP.
####################################################################
# Description:(Diferent from Part B)
####################################################################
# In our code iaddq will give us a great advantage, every time we try to add a
#constant value to a register, we used to first did irmovq operation then we made
#addq operation. But thanks to this new operation iaddq, we just combined those 
#two instruction, therefore we just reduced the number of cycles with this new
#instruction. I didn't change the control logic for this Part B, because I use
#it in a same manner as this instruction is described.
####################################################################
